...
No router library — navigation is entirely via the screen state machine. No state management library — all state lives in `App.tsx` and is passed down as props.

### Vite Dev Proxy

In dev mode, `/auth` and `/healthz` are proxied to `http://localhost:3001` (the auth backend).
